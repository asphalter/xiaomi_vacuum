...
while true; do
    if $(ls /run/shm/navmap*.ppm 1> /dev/null 2>&1) && $(ls /run/shm/SLAM_fprintf.log 1> /dev/null 2>&1); then
        ...
        if ! [ -e ${SSHCTL} ]; then
            ssh -qnNf -o ControlMaster=yes -o ControlPath="${SSHCTL}" ${RUSER}@${RHOST}
        fi
        # rsync will handle sparse files to let navmaps be transferred quickier
        rsync -cqrlDS -e "ssh -o 'ControlPath=${SSHCTL}'" --timeout 10 --include="navmap*.ppm" --include="SLAM_fprintf.log" --exclude="*" /run/shm/ ${RUSER}@${RHOST}:~/${RDIR}/
    else
        # If the robot isn't moving then exit from the SSH control path session (if exists) and sleep waiting for the next loop
        if [ -e ${SSHCTL} ]; then
            ssh -qO exit -o ControlPath="${SSHCTL}" ${RUSER}@${RHOST}
        fi
        sleep 5 
    fi
done
